...
int main(int argc, char *args[])
{
    ...
    if (!nolog) cout << "Podaj pierwszy wyraz ciagu arytmetycznego:\n";
	cin >> a1;
    cout << endl;

    if (!nolog) cout << "Podaj roznice:\n";
	cin >> r;
    cout << endl;

    if (!nolog) cout << "Podaj liczbe elementow:\n";
    cin >> n;
    cout << endl;
    dot = n;
    if (!nolog) cout << n << " elementow tego ciagu arytmetycznego to:\n";
	cout << fixed << setprecision(1);
    //ok, ponizej jest genialny przyklad jak lubie sobie utrudniac zycie
    //Pisze po angielsku, bo mysle po angielsku (weird isn't it?)
    //idea:
